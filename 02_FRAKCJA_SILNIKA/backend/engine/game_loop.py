    ...
    def _process_physics(self, agent_actions: Dict[str, Any]):
        ...
        try:
            ...
            for shooter_id in self.last_physics_results.get("shots_fired", []):
                shooter_tank = self.tanks.get(shooter_id)
                shooter_team = int(getattr(shooter_tank, "_team", 0) or 0)
                self.logger.log_tank_action(
                    shooter_id,
                    "shoot",
                    {"team": shooter_team},
            ...
            for hit in self.last_physics_results.get("projectile_hits", []):
                ...
                if hit.hit_tank_id and shooter_id:
                    ...
                    self.logger.log_tank_action(
                        ...
                        "hit",
                        {
                            "target_id": hit.hit_tank_id,
                            "damage": hit.damage_dealt,
                            "team": int(getattr(self.tanks.get(shooter_id), "_team", 0) or 0),
                        },
                    )
