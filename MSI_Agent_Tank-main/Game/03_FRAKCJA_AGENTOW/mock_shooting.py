...
# Engine-aligned ammo ranges.
AMMO_RANGES = {
    "HEAVY": 25.0,
    "LIGHT": 50.0,
    "LONG_DISTANCE": 100.0,
}
DEFAULT_AMMO_TYPE = "HEAVY"
DEFAULT_MAX_RANGE = float(AMMO_RANGES[DEFAULT_AMMO_TYPE])  # compatibility constant
MAX_AMMO_RANGE = float(max(AMMO_RANGES.values()))
...
def _validate_scene(scene: Scene) -> None:
    ...
    _require_finite("scene.ammo_range", scene.ammo_range)
    _require_finite("scene.reload_norm", scene.reload_norm)
    ...
    if scene.ammo_range <= 0.0:
        raise ValueError(f"scene.ammo_range must be > 0, got {scene.ammo_range}")
    if not (0.0 <= scene.reload_norm <= 1.0):
        raise ValueError(f"scene.reload_norm must be in [0, 1], got {scene.reload_norm}")
    if scene.can_fire and scene.reload_norm > 1e-6:
        raise ValueError(
            f"scene.can_fire=True is inconsistent with scene.reload_norm={scene.reload_norm}"
        )
...
def build_mock_features(
    enemy_visible: bool,
    enemy_dist_norm: float,
    enemy_barrel_error_norm: float,
    shot_blocked: bool,
    can_fire: bool,
    reload_norm: float,
    ammo_range: float,
) -> np.ndarray:
    """Build a feature vector for mock barrel/shoot models.

    The first four fields keep backward-compatible ordering with the old model:
    [enemy_visible, enemy_dist_norm, enemy_barrel_error_norm, shot_blocked].
    """
    _require_finite("enemy_dist_norm", enemy_dist_norm)
    _require_finite("enemy_barrel_error_norm", enemy_barrel_error_norm)
    _require_finite("reload_norm", reload_norm)
    _require_finite("ammo_range", ammo_range)

    enemy_dist_clamped = _clamp01(enemy_dist_norm)
    angle_clamped = _clamp01(enemy_barrel_error_norm)
    reload_clamped = _clamp01(reload_norm)
    ammo_range_norm = _clamp01(ammo_range / max(MAX_AMMO_RANGE, 1.0))
    ...
    return np.array(
        [
            ...
            enemy_dist_clamped,
            angle_clamped,
            1.0 if shot_blocked else 0.0,
            1.0 if can_fire else 0.0,
            reload_clamped,
            ammo_range_norm,
        ],
        dtype=np.float32,
...
def _rand_scene(
    ...
    ammo_weights: Optional[Mapping[str, float]] = None,
    ...
    can_fire_prob: float = 0.75,
    reload_block_prob: float = 0.75,
) -> Scene:
    ...
    _require_finite("can_fire_prob", can_fire_prob)
    _require_finite("reload_block_prob", reload_block_prob)
    ...
    if not (0.0 <= can_fire_prob <= 1.0):
        raise ValueError(f"can_fire_prob must be in [0, 1], got {can_fire_prob}")
    if not (0.0 <= reload_block_prob <= 1.0):
        raise ValueError(f"reload_block_prob must be in [0, 1], got {reload_block_prob}")
    ...
    ammo_dist = ammo_weights or {name: 1.0 for name in AMMO_RANGES}
    if not ammo_dist:
        raise ValueError("ammo_weights cannot be empty")
    bad_ammo = [k for k in ammo_dist if str(k).upper() not in AMMO_RANGES]
    if bad_ammo:
        raise ValueError(f"Unsupported ammo types in ammo_weights: {bad_ammo}")
    if any(float(w) < 0.0 for w in ammo_dist.values()):
        raise ValueError(f"Negative ammo weight in {ammo_dist}")
    ammo_total = float(sum(float(w) for w in ammo_dist.values()))
    if ammo_total <= 0.0:
        raise ValueError(f"ammo_weights must sum to > 0, got {ammo_dist}")
    ...
    ammo_keys = [str(k).upper() for k in ammo_dist.keys()]
    ammo_probs = np.array([float(ammo_dist[k]) for k in ammo_dist.keys()], dtype=np.float64)
    ammo_probs /= ammo_probs.sum()
    sampled_ammo_type = str(np.random.choice(ammo_keys, p=ammo_probs))
    sampled_ammo_range = float(AMMO_RANGES[sampled_ammo_type])

    shoot_range_cap = min(sampled_vision_range, sampled_ammo_range)
    ...
    if enemy_visible and random.random() < obstacle_prob:
        ...
        obstacle_blocks_shot = (
            ...
            and obstacle_dist <= min(dist, sampled_ammo_range)
        )
    if enemy_visible and random.random() < friendly_prob:
        max_block_dist = min(dist, sampled_ammo_range)
    ...
    if random.random() < can_fire_prob:
        can_fire = True
        reload_norm = 0.0
    else:
        can_fire = False
        if random.random() < reload_block_prob:
            reload_norm = random.uniform(0.05, 1.0)
        else:
            # No ammo: not reloading, but still cannot fire.
            reload_norm = 0.0
...
def _features_for_scene(scene: Scene) -> Tuple[np.ndarray, None]:
    """Returns mock features aligned with DQN-side shooting decision features."""
    _validate_scene(scene)
    ...
    feats = build_mock_features(
        enemy_visible=scene.enemy_visible,
        enemy_dist_norm=enemy_dist,
        enemy_barrel_error_norm=angle_error,
        shot_blocked=shot_blocked,
        can_fire=scene.can_fire,
        reload_norm=scene.reload_norm,
        ammo_range=scene.ammo_range,
    )
    return feats, None
...
def _shoot_label(
    ...
    if not scene.can_fire:
        return 0.0
    if scene.reload_norm > 1e-6:
        return 0.0
    if not scene.enemy_visible:
        return 0.0
    if scene.obstacle_blocks_shot or scene.friendly_blocks_shot:
        return 0.0
    effective_range = scene.ammo_range if max_range is None else min(scene.ammo_range, max_range)
    if scene.enemy_dist > effective_range:
        return 0.0
