...
class RuleBasedCombatAgent:
    ...
    def _select_ammo(
        ...
        # Dostosowanie do zasięgów: HEAVY=5, LIGHT=10, LONG=20
    ...
    def _fuzzy_shoot_decision(self, barrel_error: float, distance: float) -> bool:
        """
        Decyzja o strzale oparta na logice rozmytej.
        Bierze pod uwagę błąd celowania i odległość od celu.
        """
        abs_error = abs(barrel_error)
        
        # --- 1. Fuzyfikacja (Fuzzification) ---
        
        # Zbiory rozmyte dla błędu celowania (Error)
        # Small: Błąd jest mały (Zaostrzono z 5.0 do 3.0 stopni dla lepszej celności)
        mu_error_small = max(0.0, 1.0 - (abs_error / 3.0))
        
        # Medium: Błąd jest średni (2 - 10 stopni)
        # Funkcja trójkątna ze szczytem w 6.0
        if abs_error < 6.0:
            mu_error_medium = max(0.0, (abs_error - 2.0) / 4.0)
        else:
            mu_error_medium = max(0.0, (10.0 - abs_error) / 4.0)
            
        # Zbiory rozmyte dla odległości (Distance)
        # Close: Blisko (0 - 20 jednostek)
        mu_dist_close = max(0.0, 1.0 - (distance / 20.0))
        
        # Medium: Średnio (10 - 50 jednostek)
        if distance < 30.0:
        ...
        # Obliczamy stopień pewności strzału (Fire Confidence)
        
        # Reguła 1: Jeśli błąd jest MAŁY, to PEWNOŚĆ jest WYSOKA (1.0)
        rule1 = mu_error_small
        
        # Reguła 2: Jeśli błąd jest ŚREDNI i cel jest BLISKO, to PEWNOŚĆ jest ŚREDNIA (0.6)
        rule2 = min(mu_error_medium, mu_dist_close) * 0.6
        
        # Reguła 3: Jeśli błąd jest ŚREDNI i cel jest w ŚREDNIEJ odległości, to PEWNOŚĆ jest NISKA (0.3)
        rule3 = min(mu_error_medium, mu_dist_medium) * 0.3
        
        # Agregacja (MAX)
        fire_confidence = max(rule1, rule2, rule3)
        
        # --- 3. Defuzyfikacja / Decyzja ---
        # Próg odcięcia (Threshold)
    ...
    def _exploration_action(self, my_status: Dict[str, Any], sensor_data: Dict[str, Any]) -> ActionCommand:
        ...
        # Usunięto ograniczenia kątowe - wieża kręci się w kółko (360 stopni)
    ...
    def get_action(
        ...
        target = self._find_nearest_enemy(my_tank_status, sensor_data, current_tick)
