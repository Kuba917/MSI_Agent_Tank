...
def build_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mock-half-angle-deg", type=float, default=5.0)
    ...
    for idx in range(total_agents):
        ...
        if idx < learner_count:
            ...
            cmd = [
                ...
                str(max(0, int(args.movement_bootstrap_episodes))),
                "--movement-bootstrap-alpha-start",
                str(float(args.movement_bootstrap_alpha_start)),
                "--movement-bootstrap-alpha-end",
                str(float(args.movement_bootstrap_alpha_end)),
                "--movement-bootstrap-visible-scale",
                str(float(args.movement_bootstrap_visible_scale)),
                "--movement-bootstrap-hidden-scale",
                str(float(args.movement_bootstrap_hidden_scale)),
                "--movement-waypoint-min-distance",
                str(float(args.movement_waypoint_min_distance)),
                "--movement-waypoint-max-distance",
                str(float(args.movement_waypoint_max_distance)),
                "--movement-waypoint-lateral-max",
                str(float(args.movement_waypoint_lateral_max)),
                "--movement-waypoint-replan-ticks",
                str(max(1, int(args.movement_waypoint_replan_ticks))),
                "--movement-waypoint-reach-radius",
                str(float(args.movement_waypoint_reach_radius)),
                "--movement-waypoint-stuck-window",
                str(max(5, int(args.movement_waypoint_stuck_window))),
                "--movement-waypoint-stuck-distance",
                str(float(args.movement_waypoint_stuck_distance)),
                "--movement-enemy-zone-patrol-activate-distance",
                str(float(args.movement_enemy_zone_patrol_activate_distance)),
                "--movement-enemy-zone-patrol-exit-distance",
                str(float(args.movement_enemy_zone_patrol_exit_distance)),
                "--movement-enemy-zone-patrol-radius-min",
                str(float(args.movement_enemy_zone_patrol_radius_min)),
                "--movement-enemy-zone-patrol-radius-max",
                str(float(args.movement_enemy_zone_patrol_radius_max)),
                "--movement-enemy-zone-patrol-replan-ticks",
                str(max(1, int(args.movement_enemy_zone_patrol_replan_ticks))),
                "--movement-enemy-zone-patrol-step-deg",
                str(float(args.movement_enemy_zone_patrol_step_deg)),
                "--target-lock-lost-patience-ticks",
                str(max(0, int(args.target_lock_lost_patience_ticks))),
                "--target-lock-relaxed-shoot-margin",
                str(float(args.target_lock_relaxed_shoot_margin)),
                "--progress-reward-scale",
                str(float(args.progress_reward_scale)),
                "--exploration-reward-scale",
                str(float(args.exploration_reward_scale)),
                "--retreat-hp-threshold",
                str(float(args.retreat_hp_threshold)),
                "--mock-shoot-threshold",
                str(float(args.mock_shoot_threshold)),
                "--mock-half-angle-deg",
                str(float(args.mock_half_angle_deg)),
            ]
            if args.no_load:
                cmd.append("--no-load")
        else:
...
def build_cmd(args: argparse.Namespace, overrides: Dict[str, Any]) -> List[str]:
    ...
    add("--movement-bootstrap-episodes", overrides.get("movement_bootstrap_episodes", args.movement_bootstrap_episodes))
    add("--movement-bootstrap-alpha-start", overrides.get("movement_bootstrap_alpha_start", args.movement_bootstrap_alpha_start))
    add("--movement-bootstrap-alpha-end", overrides.get("movement_bootstrap_alpha_end", args.movement_bootstrap_alpha_end))
    add("--movement-bootstrap-visible-scale", overrides.get("movement_bootstrap_visible_scale", args.movement_bootstrap_visible_scale))
    add("--movement-bootstrap-hidden-scale", overrides.get("movement_bootstrap_hidden_scale", args.movement_bootstrap_hidden_scale))
    add("--movement-waypoint-min-distance", overrides.get("movement_waypoint_min_distance", args.movement_waypoint_min_distance))
    add("--movement-waypoint-max-distance", overrides.get("movement_waypoint_max_distance", args.movement_waypoint_max_distance))
    add("--movement-waypoint-lateral-max", overrides.get("movement_waypoint_lateral_max", args.movement_waypoint_lateral_max))
    add("--movement-waypoint-replan-ticks", overrides.get("movement_waypoint_replan_ticks", args.movement_waypoint_replan_ticks))
    add("--movement-waypoint-reach-radius", overrides.get("movement_waypoint_reach_radius", args.movement_waypoint_reach_radius))
    add("--movement-waypoint-stuck-window", overrides.get("movement_waypoint_stuck_window", args.movement_waypoint_stuck_window))
    add("--movement-waypoint-stuck-distance", overrides.get("movement_waypoint_stuck_distance", args.movement_waypoint_stuck_distance))
    add("--movement-enemy-zone-patrol-activate-distance", overrides.get("movement_enemy_zone_patrol_activate_distance", args.movement_enemy_zone_patrol_activate_distance))
    add("--movement-enemy-zone-patrol-exit-distance", overrides.get("movement_enemy_zone_patrol_exit_distance", args.movement_enemy_zone_patrol_exit_distance))
    add("--movement-enemy-zone-patrol-radius-min", overrides.get("movement_enemy_zone_patrol_radius_min", args.movement_enemy_zone_patrol_radius_min))
    add("--movement-enemy-zone-patrol-radius-max", overrides.get("movement_enemy_zone_patrol_radius_max", args.movement_enemy_zone_patrol_radius_max))
    add("--movement-enemy-zone-patrol-replan-ticks", overrides.get("movement_enemy_zone_patrol_replan_ticks", args.movement_enemy_zone_patrol_replan_ticks))
    add("--movement-enemy-zone-patrol-step-deg", overrides.get("movement_enemy_zone_patrol_step_deg", args.movement_enemy_zone_patrol_step_deg))
    add("--target-lock-lost-patience-ticks", overrides.get("target_lock_lost_patience_ticks", args.target_lock_lost_patience_ticks))
    add("--target-lock-relaxed-shoot-margin", overrides.get("target_lock_relaxed_shoot_margin", args.target_lock_relaxed_shoot_margin))
    add("--progress-reward-scale", overrides.get("progress_reward_scale", args.progress_reward_scale))
    add("--exploration-reward-scale", overrides.get("exploration_reward_scale", args.exploration_reward_scale))
    add("--retreat-hp-threshold", overrides.get("retreat_hp_threshold", args.retreat_hp_threshold))
    add("--mock-shoot-threshold", overrides.get("mock_shoot_threshold", args.mock_shoot_threshold))
    add("--mock-half-angle-deg", overrides.get("mock_half_angle_deg", args.mock_half_angle_deg))
