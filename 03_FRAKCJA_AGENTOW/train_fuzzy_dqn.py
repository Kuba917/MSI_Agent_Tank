...
- shared Team-1 policy mode (one model controls all 5 allies; one rotating learner slot).
"""
...
def build_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--shared-team1-policy",
        action="store_true",
        help="Use one shared DQN model for all Team-1 tanks (one learner slot + inference teammates).",
    )
    parser.add_argument(
        "--rotate-learner-slot",
        action="store_true",
        help="When shared-team1-policy is enabled, rotate the train-enabled Team-1 slot every episode.",
    )
    parser.add_argument(
        "--learner-slot",
        type=int,
        default=0,
        help="Fixed Team-1 learner slot (0-based) when rotate-learner-slot is disabled.",
    )
...
def parse_summary_metrics(path: Path) -> Dict[str, Any]:
    ...
    metrics: Dict[str, Any] = {
        "shots_fired": None,
        "hits_landed": None,
        "team1_shots_fired": None,
        "team1_hits_landed": None,
        "team2_shots_fired": None,
        "team2_hits_landed": None,
        "tanks_killed": None,
    ...
    patterns = {
        ...
        "team1_shots_fired": r"Team 1 Shots Fired:\s*([0-9]+)",
        "team1_hits_landed": r"Team 1 Hits Landed:\s*([0-9]+)",
        "team2_shots_fired": r"Team 2 Shots Fired:\s*([0-9]+)",
        "team2_hits_landed": r"Team 2 Hits Landed:\s*([0-9]+)",
...
def resolve_team1_learner_slot(args: argparse.Namespace, episode: int) -> Optional[int]:
    if not bool(args.shared_team1_policy):
        return None

    team_size = max(1, int(args.team_size))
    if bool(args.rotate_learner_slot):
        return (max(1, int(episode)) - 1) % team_size

    return max(0, min(int(args.learner_slot), team_size - 1))


def launch_agents(
    args: argparse.Namespace,
    episode: int,
    learner_slot: Optional[int] = None,
) -> Tuple[List[subprocess.Popen], Dict[str, Any]]:
    total_agents = args.team_size * 2
    shared_team1_policy = bool(args.shared_team1_policy)
    learner_count = 1 if shared_team1_policy else max(0, min(args.learning_agents, total_agents))
    if shared_team1_policy:
        learner_slot = resolve_team1_learner_slot(args, episode) if learner_slot is None else int(learner_slot)
        non_learner_count = max(0, int(args.team_size))
    else:
    ...
    launch_info: Dict[str, Any] = {
        "learners": 0 if shared_team1_policy else learner_count,
        "shared_team1_policy": shared_team1_policy,
        "learner_slot": int(learner_slot) if learner_slot is not None else -1,
        "shared_teammates": 0,
    ...
    for idx in range(total_agents):
        ...
        if shared_team1_policy and idx < int(args.team_size):
            team1_slot = idx
            is_learner = int(team1_slot) == int(learner_slot)
            if is_learner:
                launch_info["learners"] += 1
            else:
                launch_info["shared_teammates"] += 1

            best_for_agent = model_path_with_tag(model_path, "best")
            final_for_agent = model_path_with_tag(model_path, "final")
            cmd = [
                sys.executable,
                "DQN.py",
                "--host",
                ...
                str(port),
                "--name",
                (f"Learner_T1_{team1_slot + 1}" if is_learner else f"Team1Shared_{team1_slot + 1}"),
                "--model-path",
                str(model_path),
                "--rules",
                str(args.rules),
        ...
        elif (not shared_team1_policy) and idx < learner_count:
            ...
            if shared_team1_policy:
                opponent_idx = idx - int(args.team_size)
            else:
                opponent_idx = idx - learner_count
            use_selfplay = selfplay_enabled and opponent_idx < selfplay_requested
            if use_selfplay:
...
def summarize_episode_reports(episode_reports: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for row in episode_reports:
        ...
        team1_shots_total += int(row.get("team1_shots_fired") or 0)
        team1_hits_total += int(row.get("team1_hits_landed") or 0)
        team2_shots_total += int(row.get("team2_shots_fired") or 0)
        team2_hits_total += int(row.get("team2_hits_landed") or 0)
    ...
    team1_hit_accuracy = (team1_hits_total / team1_shots_total) if team1_shots_total > 0 else 0.0
    team2_hit_accuracy = (team2_hits_total / team2_shots_total) if team2_shots_total > 0 else 0.0
    ...
    return {
        ...
        "team1_shots_fired_total": team1_shots_total,
        "team1_hits_landed_total": team1_hits_total,
        "team2_shots_fired_total": team2_shots_total,
        "team2_hits_landed_total": team2_hits_total,
        ...
        "team1_hit_accuracy": round(team1_hit_accuracy, 4),
        "team2_hit_accuracy": round(team2_hit_accuracy, 4),
...
def main() -> int:
    ...
    if shared_team1_policy:
        slot_mode = "rotating" if bool(args.rotate_learner_slot) else f"fixed={max(0, int(args.learner_slot))}"
        print(f"Shared Team-1 policy: enabled ({slot_mode})")
    ...
    try:
        for episode in range(1, args.episodes + 1):
            ...
            learner_slot = resolve_team1_learner_slot(args, episode)
            non_learner_count = int(args.team_size) if shared_team1_policy else (total_agents - learner_count)
            ...
            role_change = bool(
                launch_info
                and (
                    desired_selfplay != int(launch_info.get("selfplay_requested", 0))
                    or int(launch_info.get("learner_slot", -1)) != (int(learner_slot) if learner_slot is not None else -1)
                    or bool(launch_info.get("shared_team1_policy", False)) != shared_team1_policy
                )
            )
            ...
            if need_restart:
                ...
                processes, launch_info = launch_agents(args, episode, learner_slot=learner_slot)
                last_launch_episode = episode

                print(
                    f"[Episode {episode}/{args.episodes}] roster: "
                    f"learners={launch_info['learners']} "
                    f"shared_teammates={launch_info.get('shared_teammates', 0)} "
                    f"learner_slot={launch_info.get('learner_slot', -1)} "
                ...
                print(
                    ...
                    f"shared_teammates={launch_info.get('shared_teammates', 0)} "
                    f"learner_slot={launch_info.get('learner_slot', -1)} "
                    f"selfplay={launch_info['selfplay']} baselines={launch_info['baselines']})"
                )
            ...
            episode_report: Dict[str, Any] = {
                ...
                "shared_team1_policy": bool(launch_info.get("shared_team1_policy", False)) if launch_info else False,
                "learner_slot": int(launch_info.get("learner_slot", -1)) if launch_info else -1,
                "shared_teammates": int(launch_info.get("shared_teammates", 0)) if launch_info else 0,
                ...
                "team1_shots_fired": _to_int((episode_metrics or {}).get("team1_shots_fired")),
                "team1_hits_landed": _to_int((episode_metrics or {}).get("team1_hits_landed")),
                "team2_shots_fired": _to_int((episode_metrics or {}).get("team2_shots_fired")),
                "team2_hits_landed": _to_int((episode_metrics or {}).get("team2_hits_landed")),
