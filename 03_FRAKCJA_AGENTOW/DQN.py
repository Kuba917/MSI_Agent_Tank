...
INSTANT_ENEMY_HIT_REWARD = 0.85
INSTANT_ENEMY_DAMAGE_REWARD_SCALE = 0.0225
...
class FuzzyDQNAgent:
    def __init__(self, name: str, config: AgentConfig, training_enabled: bool):
        ...
        self.last_model_mtime: Optional[float] = None
        self.last_reload_check_step = -10_000
    ...
    def _to_command(
        ...
        top_speed = abs(float(my_status.get("_top_speed", MAX_MOVE_SPEED) or MAX_MOVE_SPEED))
        heading_limit = abs(float(my_status.get("_heading_spin_rate", MAX_HEADING_DELTA) or MAX_HEADING_DELTA))
        barrel_limit = abs(float(my_status.get("_barrel_spin_rate", MAX_BARREL_DELTA) or MAX_BARREL_DELTA))
        move_speed = self.encoder._clamp(move_speed, -max(0.1, top_speed), max(0.1, top_speed))
        heading_rotation = self.encoder._clamp(heading_rotation, -max(0.5, heading_limit), max(0.5, heading_limit))
        barrel_rotation = self.encoder._clamp(barrel_rotation, -max(0.5, barrel_limit), max(0.5, barrel_limit))

        tank_type = str(my_status.get("_tank_type", "")).upper()
        aim_threshold_deg = 6.0
        if "HEAVY" in tank_type:
            aim_threshold_deg = 10.0
        elif "SNIPER" in tank_type:
            aim_threshold_deg = 5.0

        if obs.enemy_visible:
            if obs.enemy_dist < 0.35:
                aim_threshold_deg += 2.0
            elif obs.enemy_dist > 0.75:
                aim_threshold_deg = max(4.0, aim_threshold_deg - 1.0)

        barrel_error_signed_deg = (float(obs.enemy_barrel_error) * 2.0 - 1.0) * 180.0
        hull_error_signed_deg = (float(obs.enemy_hull_error) * 2.0 - 1.0) * 180.0
        barrel_error_deg = abs(barrel_error_signed_deg)

        # Combat override: when enemy is visible, force orientation toward target.
        if obs.enemy_visible:
            desired_barrel = self.encoder._clamp(
                (barrel_error_signed_deg / 180.0) * max(2.0, barrel_limit),
                -max(0.5, barrel_limit),
                max(0.5, barrel_limit),
            )
            if abs(desired_barrel) > abs(barrel_rotation):
                barrel_rotation = desired_barrel

            desired_heading = self.encoder._clamp(
                (hull_error_signed_deg / 180.0) * max(2.0, heading_limit),
                -max(0.5, heading_limit),
                max(0.5, heading_limit),
            )
            if abs(hull_error_signed_deg) > 10.0 and abs(desired_heading) > abs(heading_rotation):
                heading_rotation = desired_heading

            # Pursue target aggressively when it is still far away.
            if obs.enemy_dist > 0.42:
                move_speed = max(move_speed, 0.45 * max(0.1, top_speed))
            # When target is almost aligned and close, slow down but keep pressure.
            elif barrel_error_deg <= (aim_threshold_deg * 1.2):
                move_speed = self.encoder._clamp(
                    move_speed * 0.55,
                    0.25 * max(0.1, top_speed),
                    max(0.1, top_speed),
                )
        else:
            # Periodic search sweep when target is not visible.
            scan_interval = 14
            if "HEAVY" in tank_type:
                scan_interval = 18
            elif "SNIPER" in tank_type:
            ...
            scan_step = max(0.8, min(max(0.5, barrel_limit), 3.5))
            half = max(1, scan_interval // 2)
            phase = int(current_tick) % scan_interval
            if abs(barrel_rotation) < (0.2 * max(1.0, barrel_limit)):
                if phase in (0, 1):
                    barrel_rotation = scan_step
                elif phase in (half, half + 1):
                    barrel_rotation = -scan_step

            heading_scan = self.encoder._clamp(
                0.35 * max(0.5, heading_limit),
                -max(0.5, heading_limit),
                max(0.5, heading_limit),
            )
            if abs(heading_rotation) < (0.12 * max(1.0, heading_limit)):
                if phase < half:
                    heading_rotation = heading_scan
                else:
                    heading_rotation = -heading_scan

            if abs(move_speed) < (0.25 * max(0.1, top_speed)):
                move_speed = max(0.35 * top_speed, 0.4)

            # Push toward enemy side to prevent local loops and camping.
            team = int(my_status.get("_team") or 0)
            pos = my_status.get("position", {}) or {}
            pos_x = float(pos.get("x") or 0.0)
            min_attack_speed = 0.55 * max(0.1, top_speed)
            if team == 1 and pos_x < (MAP_WIDTH * 0.72):
                move_speed = max(move_speed, min_attack_speed)
            elif team == 2 and pos_x > (MAP_WIDTH * 0.28):
                move_speed = min(move_speed, -min_attack_speed)

        # Aggressive trigger: when enemy is visible and fire lane is safe, keep firing.
        fire_allowed = obs.can_fire and obs.enemy_visible and (not obs.ally_fire_risk)
        should_fire = bool(fire_allowed)
    ...
    def _compute_step_reward(
        ...
        parts["enemy_dist_progress"] = 0.0
        parts["spotted_enemy"] = 0.0
        parts["lost_enemy"] = 0.0
        if current_obs.enemy_visible:
            parts["enemy_visible"] = 0.45
            ...
            if prev_obs.enemy_visible:
                parts["enemy_dist_progress"] = (prev_obs.enemy_dist - current_obs.enemy_dist) * 2.0
            else:
                parts["spotted_enemy"] = 0.45
        else:
            parts["enemy_visible"] = -0.05
            parts["barrel_improve"] = 0.0
            parts["hull_improve"] = 0.0
            if prev_obs.enemy_visible:
                parts["lost_enemy"] = -0.45
        ...
        parts["powerup_pursuit"] = (
            (prev_obs.powerup_dist - current_obs.powerup_dist) * 0.03
            if current_obs.powerup_visible
            else 0.0
        )

        parts["close_combat_presence"] = 0.0
        if current_obs.enemy_visible:
            parts["close_combat_presence"] = max(0.0, 0.55 - current_obs.enemy_dist) * 0.25
        ...
        parts["blind_turn"] = 0.0
        if (
            not current_obs.enemy_visible
            and abs(action.move_speed) < 0.2
            and (abs(action.heading_rotation_angle) + abs(action.barrel_rotation_angle) > 4.5)
        ):
            parts["blind_turn"] = -0.08

        parts["search_pattern"] = 0.0
        parts["no_enemy_stall"] = 0.0
        if not current_obs.enemy_visible:
            if abs(action.move_speed) > 0.35 and abs(action.heading_rotation_angle) > 0.4:
                parts["search_pattern"] = 0.12
            if abs(action.move_speed) < 0.15 and abs(action.heading_rotation_angle) < 0.2:
                parts["no_enemy_stall"] = -0.12

        # Reward displacement toward enemy side to reduce passive local loops.
        parts["enemy_side_progress"] = 0.0
        if len(self.pos_history) >= 2 and my_team in (1, 2):
            prev_pos = self.pos_history[-2]
            dx = current_pos[0] - prev_pos[0]
            toward_enemy_dx = dx if my_team == 1 else -dx
            parts["enemy_side_progress"] = toward_enemy_dx / 25.0
        parts["home_camp_penalty"] = 0.0
        if my_team in (1, 2) and current_tick > 180:
            if my_team == 1 and current_pos[0] < (MAP_WIDTH * 0.34):
                parts["home_camp_penalty"] = -0.25
            if my_team == 2 and current_pos[0] > (MAP_WIDTH * 0.66):
                parts["home_camp_penalty"] = -0.25
        ...
        parts["variance_recent"] = 0.0
        parts["variance_prev"] = 0.0
        parts["centroid_bonus"] = 0.0
        parts["camp_penalty"] = 0.0
        if len(recent) >= 50:
            span_x = max(p[0] for p in recent) - min(p[0] for p in recent)
            span_y = max(p[1] for p in recent) - min(p[1] for p in recent)
            if (span_x + span_y) < 6.0:
                parts["camp_penalty"] = -0.6
        ...
        if self.frontier_min_x is None:
            ...
            if current_pos[0] < self.frontier_min_x:
                frontier_bonus += (self.frontier_min_x - current_pos[0]) / 80.0
                self.frontier_min_x = current_pos[0]
            if current_pos[0] > self.frontier_max_x:
                frontier_bonus += (current_pos[0] - self.frontier_max_x) / 80.0
                self.frontier_max_x = current_pos[0]
            if current_pos[1] < self.frontier_min_y:
                frontier_bonus += (self.frontier_min_y - current_pos[1]) / 80.0
                self.frontier_min_y = current_pos[1]
            if current_pos[1] > self.frontier_max_y:
                frontier_bonus += (current_pos[1] - self.frontier_max_y) / 80.0
                self.frontier_max_y = current_pos[1]
    ...
    def _checkpoint_payload(self) -> Dict[str, Any]:
        return {
            ...
            "reward_parts_history": self.reward_parts_history,
            "config": {
    ...
    def _load_checkpoint_if_available(self) -> None:
        ...
        try:
            ...
            if isinstance(checkpoint, dict) and "actor_state_dict" in checkpoint:
                ...
                reward_history = checkpoint.get("reward_parts_history")
                if isinstance(reward_history, dict):
                    ...
                    for key, values in reward_history.items():
                        ...
                        if clean_values:
                            restored[str(key)] = clean_values
                    if restored:
                        self.reward_parts_history = restored
            else:
                print(f"[{self.name}] checkpoint format not recognized; starting fresh.")

            print(f"[{self.name}] checkpoint loaded: {path}")
            try:
                self.last_model_mtime = os.path.getmtime(path)
            except OSError:
                self.last_model_mtime = None
        except Exception as exc:
            print(f"[{self.name}] failed to load checkpoint {path}: {exc}")

    def _maybe_reload_inference_checkpoint(self) -> None:
        if self.training_enabled:
            return
        path = self.config.model_path
        if not path or not os.path.exists(path):
            return
        if (self.total_steps - self.last_reload_check_step) < 40:
            return
        self.last_reload_check_step = self.total_steps
        try:
            mtime = os.path.getmtime(path)
        except OSError:
            return
        if self.last_model_mtime is not None and mtime <= self.last_model_mtime:
            return
        ...
        try:
            ...
            print(f"[{self.name}] inference model reloaded: {path}")
        except Exception as exc:
            print(f"[{self.name}] inference reload failed: {exc}")
    ...
    def get_action(
        ...
        with self.lock:
            self._maybe_reload_inference_checkpoint()
            ...
            command = self._to_command(action_vec, my_tank_status, current_obs, current_tick)
    ...
    def end(self, damage_dealt: float, tanks_killed: int) -> None:
        with self.lock:
            final_reward = tanks_killed * 7.0 + (damage_dealt / 45.0)
            ...
            if self.training_enabled and self.games_played % max(1, self.config.save_every_games) == 0:
                ...
                # best_score and reward history persist across process restarts.
    ...
    def _save_reward_plot(self) -> None:
        ...
        out_dir = os.path.join(current_dir, "training_reports")
        os.makedirs(out_dir, exist_ok=True)
        if self.training_enabled and self.config.model_path:
            stem = os.path.splitext(os.path.basename(self.config.model_path))[0]
            out_path = os.path.join(out_dir, f"rewards_{stem}.png")
            snapshot_path = os.path.join(out_dir, f"rewards_{stem}_game_{self.games_played}.png")
        else:
        ...
        if snapshot_path:
            print(f"[{self.name}] reward plot saved: {out_path} | snapshot: {snapshot_path}{suffix}")
        else:
