...
class FuzzyDQNAgent:
    ...
    def _to_command(
        ...
        if obs.enemy_visible:
            ...
            # When target is almost aligned, reduce speed to stabilize aim and improve hit chance.
            if barrel_error_deg <= (aim_threshold_deg * 1.2):
        ...
        fire_allowed = (
            obs.can_fire
            and obs.enemy_visible
            and (not obs.ally_fire_risk)
            and (barrel_error_deg <= aim_threshold_deg)
        )
        # Deterministic trigger discipline: shoot immediately only when locked.
    ...
    def _compute_step_reward(
        ...
        if current_obs.enemy_visible:
            ...
            if prev_obs.enemy_visible:
                parts["enemy_dist_progress"] = (prev_obs.enemy_dist - current_obs.enemy_dist) * 1.2
            else:
                parts["spotted_enemy"] = 0.3
        else:
            parts["enemy_visible"] = -0.05
            parts["barrel_improve"] = 0.0
            parts["hull_improve"] = 0.0
            if prev_obs.enemy_visible:
                parts["lost_enemy"] = -0.25
        ...
        if len(self.pos_history) >= 2 and my_team in (1, 2):
            ...
            parts["enemy_side_progress"] = toward_enemy_dx / 40.0
        parts["home_camp_penalty"] = 0.0
        if my_team in (1, 2) and current_tick > 280:
            if my_team == 1 and current_pos[0] < (MAP_WIDTH * 0.34):
                parts["home_camp_penalty"] = -0.12
